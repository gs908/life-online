...
def _to_read(u) -> UploadRead:
    return UploadRead(
        id=u.id, object_key=u.object_key, bucket=u.bucket,
        content_type=u.content_type, size=u.size, purpose=u.purpose,
        access_url=upload_service.build_access_url(u.object_key),
    )
